...
import { CustomerOrderService, OrderService } from './order.service';
type PaginatedOrder = {
  data: Order[];
  paginatorInfo: any;
};
...
const useOrdersQuery = (
  ...
  return useInfiniteQuery<PaginatedOrder, Error>(
    [`${API_ENDPOINTS.ORDERS}/customer`, params],
    fetchOrders,
    {
      ...options,
...
export const fetchSimpleOrderStatuses = async () => {
  const {
    data,
  } = await OrderService.fetchUrl(`${API_ENDPOINTS.ORDER_STATUS}/user`);
  return {
    order_statuses: { data },
  };
};
...
export const useOrderStatusesQuery = () => {
  return useQuery<any, Error>(`${API_ENDPOINTS.ORDER_STATUSES}/user`, fetchSimpleOrderStatuses);
};
